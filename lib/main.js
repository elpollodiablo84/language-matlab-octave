    ...
    sectionFold () {
      ...
      if (editor && editor.getGrammar().scopeName === 'source.matlab') {
        ...
        for (let i = 0; i < L; i++) {
          ...
          var line = editor.lineTextForBufferRow(i)
          var nextLine = editor.lineTextForBufferRow(iNext)

          var nextLineIsComment = nextLine.trim().startsWith('%%')
          var nextLineIsEnd = nextLine.search(/^\s*end\s/m) > 0
          var nextLineIsMidBlock = nextLine.search(/^\s*else\s/m) > 0 ||
                                   nextLine.search(/^\s*elseif\s/m) > 0 ||
                                   nextLine.search(/^\s*catch\s/m) > 0 ||
                                   nextLine.search(/^\s*case\s/m) > 0
